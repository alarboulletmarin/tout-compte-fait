...
const backOneMonth = (page: Page): Locator =>
  page.getByRole("button", { name: /mois précédent/i });
...
async function heading(page: Page): Promise<string> {
  return (await (await monthHeader(page)).innerText())
    .split("\n")
    .slice(0, 2)
    .join(" · ");
}
...
async function firstPlannedRow(page: Page): Promise<Locator> {
  ...
  await expect(page.locator('button[aria-label^="Confirmer"]')).not.toHaveCount(
    0,
  );
...
test("le bloc titre ramène au mois courant quand on le tape", async ({
  page,
}) => {
  await openApp(page);
  ...
  await expect(page.getByRole("button", { name: /^revenir à /i })).toHaveCount(
    0,
  );
});

test("le retour nomme son année dès qu’elle n’est plus celle qu’on lit", async ({
  page,
}) => {
  await openApp(page);
  await loadExample(page);
  ...
  await expect(
    page.getByRole("button", { name: /^revenir à /i }),
  ).toContainText(/revenir à \p{L}+ \d{4}/u);
});

test("une rangée encore à confirmer ouvre sa fiche au clic", async ({
  page,
}) => {
  await openApp(page);
  await loadExample(page);
...
test("le glissé confirme, et son fond fait la hauteur de la rangée", async ({
  page,
}) => {
  await openApp(page);
  await loadExample(page);
  ...
  const label = await row
    .locator('button[aria-label^="Confirmer"]')
