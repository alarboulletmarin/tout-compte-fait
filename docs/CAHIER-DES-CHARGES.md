...
- Projections d'épargne : un simulateur à taux constant, jusqu'à trois hypothèses comparées, en euros courants ou constants
...
**Hors périmètre** — plans d'épargne branchés sur le réel, objectifs d'épargne datés et suivis, allocation de portefeuille, conseil d'investissement, modèle fiscal français (enveloppes, prélèvement forfaitaire, prélèvements sociaux) et presets de frais par support, simulation stochastique (Monte Carlo), données de marché, comptes bancaires multiples, import de relevés bancaires, synchronisation bancaire, budgets par enveloppe, multi-devise, patrimoine hors épargne, solde roulant entre membres (une dette entre personnes qui court de mois en mois jusqu'à ce qu'un geste la solde ; la régularisation, elle, corrige le mois suivant et s'arrête là).

**Les projections sont entrées dans le périmètre, et pas les plans.** La liste disait « plans et projections d'épargne, rendement attendu, intérêts composés, scénarios prudent/central/optimiste », c'est-à-dire cinq façons de nommer un seul chantier — et l'argument qui les tenait dehors était juste pour un seul d'entre eux. Un **plan** *suit* les données : il compare le prévu au confirmé mois après mois, donc il ajoute une entité au document, des migrations, et une lecture de plus sur chaque écran d'épargne. Une **projection** capitalise quatre nombres et s'arrête là — elle peut les *lire* dans le document sans rien y écrire, ce qui est une opération sans conséquence, quand suivre en est une. Ce n'est pas un demi-plan, c'est un autre objet — et le refuser laissait à l'app un trou qu'aucun autre écran ne comble, alors que les outils qui le comblent ailleurs le font mal exprès (§4.6 ter).

Le chantier des **plans** reste donc entier, et le modèle continue de les accueillir sans les implémenter : un plan viserait un ou plusieurs `SavingSupport`, partirait de leur dernière valorisation, ajouterait les versements déjà déclarés, appliquerait une hypothèse de rendement, et comparerait la projection aux valorisations réelles. Rien de tout cela n'est codé, et **aucun champ n'est posé « au cas où »** : un `expectedReturn` inutilisé sur un support serait une promesse dans le modèle, lue par un assistant et par la documentation comme un réglage qui fait quelque chose. Le simulateur ne l'a pas changé. Il en a pris les deux premiers pas — partir d'un support, reprendre ses versements déclarés — et laissé les deux derniers : il n'**écrit rien** dans le document, et il ne compare **rien** au réel. C'est ce qui reste à faire qui coûte des migrations, pas ce qui vient d'être fait (§4.6 ter).

La **Monte Carlo** est le seul point de la liste que le simulateur aurait pu prendre et ne prend pas. Une distribution de trajectoires est plus honnête qu'un taux constant, et strictement moins explicable : « il y a 70 % de chances d'atteindre 180 k€ » demande d'admettre un modèle de rendement qu'on ne peut ni montrer ni justifier ici. Trois hypothèses comparées disent la même incertitude avec des nombres que le lecteur peut refaire à la main.
...
3. **Pourquoi mon mois ressemble à ça** — où part l'argent, ce qui de ce montant est à soi, ce qu'on peut mettre de côté, comment le foyer se répartit, ce qu'on doit encore ; puis ce qui tombe bientôt, puis le détail des lignes.
...
- **Perso et commun**, sous un filtre par membre seulement : ce que le mois coûte à la personne filtrée, éclaté en ses charges à elle et sa part du pot commun. Ses chiffres comprennent déjà cette part (§4.7 ter) — sans quoi elle se lirait comme si elle vivait sans loyer —, mais une fois fondue dans le total des charges, elle ne se voit plus : le solde valait bien ses revenus moins ses charges moins sa part du commun, et le troisième terme n'apparaissait nulle part. Elle vient donc juste après « Où part l'argent », dont elle est l'autre découpe du même montant — par famille chez l'une, par ce qui se décide seul·e ou à deux chez l'autre. Son total est celui de la tuile Charges, au centime : elle ne le contredit pas, elle l'éclate. Les montants s'affichent au centime pour cette raison précise — arrondies, les deux moitiés ne redonneraient plus le total annoncé par la tuile voisine. Elle ne dit pas un mot du virement : un coût est arrêté au mois où la dépense a eu lieu, un virement se rattrape (§4.7 ter), et les mêler dans une carte fait lire une soustraction qui ne tombe pas.
- **À verser sur le commun**, sous un filtre par membre seulement : ce que la personne filtrée doit virer sur le compte joint, le coefficient qui le produit, et le calcul qui y mène — sa part du mois, plus la régularisation du mois précédent. La tuile sert un geste, et un seul : on vient y recopier une somme dans une application bancaire. Elle a longtemps porté en plus ses charges perso et le coût de son mois, c'est-à-dire deux montants qui ne sont pas des virements : le report entrait dans le chiffre de tête et pas dans eux, si bien qu'un « Total à payer » s'affichait plus petit que le « À verser » posé juste au-dessus — ils sont passés à « Perso et commun ». Le report, lui, avait sa propre tuile alors qu'il était déjà compris — silencieusement — dans le chiffre de tête : deux montants voisins dont rien ne dit qu'ils ne s'ajoutent pas s'ajoutent. Il est redevenu un terme du calcul, sur la tuile dont il change le montant, et ne se lit qu'aux mois où il existe. C'est la contrepartie exacte de la tuile Répartition, qui montre les parts de tout le monde et s'efface sous un filtre : l'une ou l'autre est visible, jamais les deux, et elles mènent au même écran de détail. Elle vaut aussi pour qui est seul·e : sa part est le pot entier, à 100 % (§4.7 ter). Elle reste debout le mois où aucune charge n'est commune mais où un report attend — c'est le mois où le virement n'est plus que lui, donc celui où il faut le lire —, et son montant garde alors son signe : qui a trop avancé reçoit au lieu de verser.
...
**Le calcul se replie.** La cascade qui produit la capacité — revenus, charges, crédits — reste indispensable : un crédit qui mange la moitié de la capacité ne se voit qu'ici, le tableau de bord le fond dans « Charges ». Sous un filtre, elle **nomme son troisième terme** : ce que la personne porte seule d'un côté, sa part du pot commun de l'autre — crédits communs compris —, parce que « Charges » d'un bloc rendait le seul terme sur lequel on peut agir seul·e indiscernable de celui qui se décide à deux, sur l'écran qui sert justement à décider quoi changer. Les termes continuent de redonner la capacité au centime : ce sont les mêmes totaux, seulement séparés. Tant que le prorata ne se calcule pas, il n'y a aucune part à distinguer et la cascade reprend ses termes d'origine. Mais c'est une **vérification**, pas une lecture qu'on vient chercher chaque mois, et affichée en permanence avec les quatre paragraphes de méthode elle occupait le dernier tiers de l'écran. Elle vit sous « Comprendre le calcul », repliée, avec la règle qui fait exister ce § — un relevé n'est pas un mouvement.
...
### 4.6 ter Projections — le simulateur

L'app suivait l'épargne **réalisée** — capacité, ventilation, capital relevé — et ne projetait rien. Les simulateurs qui comblent ce trou ailleurs le font à l'envers, et exprès : taux figé et flatteur (11 % « constaté sur 2016-2025 », c'est-à-dire une des meilleures décennies de l'histoire du MSCI World), euros courants, précision à l'euro sur vingt ans. Ce sont des outils de vente, et chacun de ces trois choix vend. **Ici il n'y a rien à vendre**, donc rien qui oblige à promettre — c'est la seule raison pour laquelle cet écran a le droit d'exister, et c'est elle qui décide de chacune de ses règles.

**L'écran n'écrit rien dans le document, et il ne le lit que dans un sens.** Rien de ce qu'on y tape ne ressort dans un export ni dans le schéma qu'on donne à un assistant, aucune `Entry` n'y naît, aucun relevé n'y est touché : une projection est une **question qu'on pose**, pas un fait du foyer, et la réponse change dès qu'on change d'avis sur le taux. Les derniers réglages sont gardés en confort local, hors du document, comme le thème : ce qui décrit cet appareil-ci ne voyage pas dans un fichier exporté. Le pire qu'on risque à les perdre est de retaper trois chiffres.

Il ne **lisait** pas davantage qu'il n'écrivait, et cette symétrie-là était fausse. Refuser d'écrire protège le document ; refuser de lire ne protégeait rien — ça obligeait seulement à retaper à la main un capital que l'écran Épargne affiche au centime deux écrans plus haut. Recopier une donnée qu'on possède n'est pas de la prudence, c'est une corvée, et une corvée qui produit des fautes de frappe. L'écran lit donc, sous trois règles :

- **Le sens est unique.** Ce qui est repris s'affiche en **lecture**, jamais dans un champ de saisie : on ne tape pas par-dessus l'épargne, donc on ne peut pas croire qu'on la modifie. « Modifier pour cette simulation » recopie les valeurs dans la saisie et **coupe le lien** — c'est la seule façon d'y toucher, et elle consiste à cesser de lire.
- **Le rendement n'est jamais repris.** Le capital et les versements sont des faits ; un rendement futur n'en est pas un, et aucun support n'en porte. Un Livret A dont le taux du jour est connu n'est pas un « rendement garanti sur dix ans » : il est révisé deux fois par an. Préremplir le taux depuis un support serait très exactement le tour de passe-passe que cet écran existe pour ne pas faire.
- **Rien ne s'additionne entre deux personnes.** Une origine désigne un support, ou toute l'épargne d'**une** personne — jamais un total du foyer (§4.6 bis). Le capital repris est la valeur **estimée**, celle que la couverture divise déjà : deux réponses à « combien j'ai aujourd'hui » d'un écran à l'autre seraient indéfendables. Un support sans relevé n'est **pas** compté à zéro, il est compté à part et l'écran le dit ; une règle au montant variable n'a pas de mensualité à reprendre, et l'écran le dit aussi.

**L'ordre de l'écran : la réponse avant la question.** Il allait explication → paramètres → taux → inflation → résultat → jalons, c'est-à-dire presque tout l'écran à traverser avant « ≈ 14 k€ », qui est pourtant la seule chose qu'on vient y chercher. On n'ouvre pas `/projections` pour remplir un formulaire. Il va donc **résultat → paramètres → hypothèses → tracé → détails** : les paramètres servent à ajuster la réponse, pas à y accéder.

**Les entrées.**

- **Point de départ** : simulation libre, un support, ou toute l'épargne d'une personne. Libre par défaut, et jamais l'épargne : un écran qui s'ouvrirait sur « Livret A » aurait choisi à la place de qui le lit quel compte mérite d'être projeté. Une origine gardée qui ne désigne plus rien — support supprimé, document remplacé à l'import — retombe en libre plutôt que d'afficher une épargne vide sous le nom d'un compte disparu.
- **Capital de départ**, zéro par défaut ; **versement mensuel** ; **durée**, avec quatre raccourcis — 5, 10, 15, 20 ans — et un champ libre borné à 50 ans. Le champ reste la vérité — sans lui, un horizon de sept ans serait inatteignable — mais il ne s'affiche **que sur demande** : deux contrôles pour une seule donnée, côte à côte en permanence, en font un de trop, et celui qui doit se demander est celui dont on se sert le moins. Une durée gardée qui n'est pas un raccourci ramène son champ avec elle, sans quoi elle reviendrait à l'écran sans rien pour la relire. Cinquante ans est une borne de **sens**, pas de calcul — au-delà, ni le taux, ni l'inflation, ni la personne qui lit ne seront les mêmes.
- **Chaque champ porte son unité**, à côté de lui — `€`, `€/mois`, `%/an`. « 0 », « 100 » et « 3 » empilés dans une colonne ne disent pas lequel est un montant et lequel un pourcentage ; le libellé le dit, mais il est au-dessus, et l'œil qui relit ses chiffres ne remonte pas. À côté du champ et non dedans : les montants sont alignés à droite (DS §3), et un suffixe posé à l'intérieur tomberait pile sur le dernier chiffre tapé.
- **Taux annuel, saisi net de frais et de fiscalité.** L'app ne modélise ni prélèvement forfaitaire, ni prélèvements sociaux, ni frais de gestion : la feuille d'explication dit comment estimer son taux net — le rendement espéré, moins les frais annuels, moins la fiscalité applicable aux gains. C'est une simplification **assumée et écrite** : la fiscalité française bouge, et un barème figé dans le code serait faux dans deux ans. Un barème périmé se lit comme un calcul faux.
- **Type de taux** : **taux garanti** (contractuellement, sur toute la durée simulée) ou **rendement hypothétique** (PEA, compte-titres, unités de compte). Il ne change aucun calcul — il change ce que le chiffre **engage** : un taux garanti est une propriété du contrat, une hypothèse n'engage que qui la pose. La distinction passe par le mot et par la forme du trait, **jamais par la couleur seule** (DS §2.3). Les libellés disaient « Garanti / Hypothèse », ce qui laissait entendre que l'app savait lequel des deux était vrai : elle ne sait rien du contrat, et c'est la personne qui coche qui l'affirme. La mise en garde n'apparaît que sur « taux garanti », parce que c'est là qu'elle corrige quelque chose — « rien n'est promis » sous « rendement hypothétique » ne fait que répéter le libellé.
- **Le taux par défaut est une hypothèse à 3 %, et jamais un taux garanti.** Écrire un taux garanti reviendrait à annoncer celui d'un produit — un livret réglementé est révisé au 1er février et au 1er août, donc un chiffre en dur serait daté par construction et faux dans les six mois. Une hypothèse n'engage que la personne qui la valide, et 3 % est modeste : c'est le contraire exact du chiffre que présélectionne un simulateur de vente.

**Scénarios.** Jusqu'à **trois** hypothèses comparées, aux mêmes versements. La limite est dure, et elle a deux fondements qui tombent au même endroit : au-delà, les courbes superposées ne se lisent plus sur un téléphone, et l'app n'a que **trois** couleurs de trait qui tiennent le contraste de 3:1 dans les deux thèmes. C'est aussi pourquoi les versements cumulés sont une **aire** et non une quatrième courbe.

**Mode inverse.** Objectif — montant visé et horizon — → **versement mensuel requis**, une hypothèse à la fois. Plus actionnable que « combien j'aurai » : c'est la question qu'on se pose vraiment. Le versement requis s'arrondit au centime **supérieur**, seul chiffre de l'écran dans ce sens : arrondi par le bas il raterait sa cible, de peu mais toujours du même côté. Quand le capital de départ atteint seul la cible, l'écran le **dit** — « 0,00 € par mois » n'est pas une réponse.

**Les sorties.**

- **Le résultat en tête d'écran** : le chiffre d'arrivée, puis sa **décomposition** — capital de départ, versements, rendement, capital projeté — puis la part que le rendement pèse dans le total, puis les hypothèses en une ligne. Un capital projeté est trois choses, et un nombre seul les confond : « ≈ 14 000 € » impressionne, « 12 000 € versés et 1 900 € de rendement » informe. C'est la seule pédagogie que cet écran ait à donner, et elle rend inutile la tuile « ce que le taux aura produit » qui vivait tout en bas et disait la moitié de la phrase. En mode inverse, le héros est le **versement requis** : c'est la réponse, et le capital d'arrivée y serait le chiffre qu'on vient de taper.
- Une **courbe par scénario**, et l'aire des **versements cumulés** en dessous. À **une seule hypothèse**, les deux se lisent **empilées** — versements en bas, rendement au-dessus, le haut de la pile étant le capital : la hauteur de chaque bande *est* la réponse, là où deux traits superposés demandaient de mesurer l'écart entre eux. Au-delà d'une, on n'empile plus : trois rendements posés sur le même versé ne s'additionnent pas, la comparaison redevient le propos, et c'est un trait par hypothèse. L'échelle part de **zéro**, contrairement à la courbe d'un support (§4.6 bis), et elle le doit : une aire mesurée depuis une base flottante ne dit rien.
- **Le tracé se lit au doigt et au clavier.** Un curseur (`charts/ChartCursor`, celui de l'historique) déplace la lecture le long de l'horizon, et la légende au-dessus donne à ce rang-là la durée, le capital et le versé. Les arrêts sont plafonnés à vingt-quatre : une période de lecture se vise, et cent vingt en donneraient deux pixels chacune. L'écart aux 44px du DS §8 est le même que celui, déjà mesuré, des douze mois de l'historique — il est tenu par le clavier, par le tableau et par le résumé.
- Un **tableau de jalons**, un rang par quart d'horizon, **derrière un repli**. Il n'est pas un doublon : une courbe ne se lit pas au chiffre près, et c'est lui la lecture textuelle que le §5 exige de tout graphique. Il se replie parce que le curseur répond désormais à la même question au doigt : il n'est plus la seule lecture précise, il est la lecture exhaustive. À **une hypothèse**, ses colonnes portent le **versé**, le **rendement** et le **total** plutôt qu'une colonne de totaux et rien d'autre ; à plusieurs, une colonne par hypothèse. Des quarts d'horizon plutôt qu'une liste fixe 5/10/15/20 : sur quarante ans une liste fixe laisserait vingt années sans repère, sur trois ans elle ne rendrait rien — et sur vingt ans les quarts **redonnent** 5/10/15/20.
- **« Et si je verse davantage ? »** — quatre versements autour de celui qu'on simule, et ce qu'ils donnent au même horizon sous la première hypothèse. C'est la seule lecture **actionnable** du mode direct : « combien j'aurai » se contemple, « ce que 150 € de plus changeraient » se décide, et la capitalisation étant convexe, l'intuition se trompe toujours dans le même sens. Les barreaux sont des **multiples du versement réel** arrondis à un pas lisible, jamais des paliers que l'app jugerait raisonnables : il n'y a ni montant conseillé, ni profil. Le versement simulé reste dans la liste, à son montant exact et marqué comme tel. Muette en mode inverse, où le versement requis répond déjà à la question par l'autre bout.
- **Montants arrondis**, « ≈ 202 k€ », jamais à l'euro sur un gros chiffre : **la précision affichée ne dépasse pas celle du modèle**. Le centime affiché est précisément ce qui fait passer une hypothèse pour une mesure. Le « ≈ » est sur chaque cellule et non une fois en légende — un montant recopié hors de son tableau doit emporter avec lui le fait qu'il sort d'un modèle.
- **Le « ≈ » commence à la première capitalisation, et pas avant.** Ce qui **entre** dans le calcul s'écrit exactement : un objectif tapé, un versement programmé, un capital relevé sont des faits. Arrondir le capital de départ à « ≈ 24 k€ » sous un panneau qui vient d'annoncer « 23 600 € » donnerait deux réponses à la même question à trois centimètres d'écart, et ferait passer pour une estimation ce que l'écran Épargne affiche au centime.
- Option **euros constants**, déflatés d'une inflation paramétrable (2 % par défaut). Désactivée par défaut, et **signalée en toutes lettres quand elle est active** : tous les chiffres de l'écran changent alors de sens. Elle se nomme par son effet — « tenir compte de l'inflation », et « affiche le pouvoir d'achat équivalent » — puis se montre sur les chiffres qu'on a sous les yeux : « ≈ 14 000 € dans 10 ans correspondraient à environ ≈ 11 500 € d'aujourd'hui ». « Déflate les montants de l'inflation » est exact et n'apprend rien à qui ne sait pas déjà ce qu'est déflater. Chaque montant se déflate **au rang de sa date**, jamais à la fin : le versé cumulé est la somme de versements déjà déflatés, pas la déflation de leur somme, et seul le premier répond à « qu'ai-je mis, en euros d'aujourd'hui ». En mode inverse, la cible est **réinflatée** avant le calcul — qui tape « 100 000 € » en lisant en euros d'aujourd'hui parle du pouvoir d'achat qu'il connaît, pas d'un nombre affiché sur un relevé dans vingt ans. Le taux saisi est net de frais et d'impôt, mais **jamais net d'inflation** : ce sont deux couches, et l'option les sépare au lieu de les confondre.

**Wording.** « Projection à taux constant », jamais « performance ». Une phrase fixe, qui ne se replie ni ne s'écarte, rappelle qu'un taux moyen n'est pas une trajectoire et que rien ici n'est une promesse ni un conseil : c'est la seule chose de l'écran qui reste vraie quels que soient les chiffres saisis. Elle vit **sous le résultat**, à l'endroit où elle est lue au moment où elle sert.

**Le reste de la pédagogie est derrière une seule porte.** Sept paragraphes vivaient dans le flux — le taux constant, le taux net, la fiscalité, la convention de fin de mois, l'inflation, les arrondis, ce que l'écran fait des données. Chacun était juste ; ensemble ils faisaient une notice intercalée entre des champs de saisie, et personne ne lit une notice. « Comprendre cette projection » les rassemble dans une feuille, d'un seul tenant, là où on les lit une fois. La réserve, elle, n'y est **pas descendue** : une mise en garde qu'il faut ouvrir pour lire n'en est plus une.
...
Il n'est **pas** un cinquième rang de « Gérer », et c'est la distinction qui fait exister le groupe : « Gérer » range ce qui décide de ce que le budget calcule, quand un simulateur ne décide de rien — il lit le document, il ne le change jamais. Le titre est un verbe comme les autres, « Simuler » et non « Calculateurs » : les quatre autres nomment une **intention**, quand « Calculateurs » nommerait une catégorie d'outil, c'est-à-dire ce qu'on a sous la main plutôt que ce qu'on cherche. C'est le reproche exact fait à « Réglages », et le rangement ne le réintroduit pas par la petite porte.
...
- **Taux mensuel = (1 + r)^(1/12) − 1. Jamais r/12.** L'écart n'est pas un arrondi : à 11 %, le taux proportionnel capitalisé douze fois rend 11,57 % par an, soit 216 k€ au lieu de 202 sur 250 €/mois pendant vingt ans — quatorze mille euros sortis de nulle part, et toujours dans le sens qui flatte. `domain/debt.ts` fait pourtant l'inverse, et ce n'est pas une incohérence : un prêt immobilier français **est** contractuellement défini au taux nominal proportionnel, où `r/12` est la convention exacte. Ici il n'y a pas de contrat — on capitalise un rendement.
- **Capitalisation mensuelle, versements en fin de mois.** Convention prudente, et c'est le motif du choix : un versement en début de mois passerait un mois de plus à produire des intérêts et rendrait un chiffre supérieur pour le même effort. Entre deux conventions défendables, l'app prend celle qui promet le moins. C'est aussi celle des simulateurs du marché, ce qui rend le vecteur de référence vérifiable ailleurs qu'ici.
- **Euros constants : montant / (1 + i)^t**, `i` l'inflation annuelle, `t` en années.
- **Vecteur de référence** : 250 €/mois, 11 %, 20 ans → **202 136,25 €**, soit ≈ 202 k€. Il fixe la convention autant qu'il vérifie le calcul — en début de mois il donnerait 203,9 k€.
- **Un seul moteur.** Le tracé, les jalons, la décomposition, l'échelle des efforts et le chiffre d'arrivée lisent la **même** série ; il n'existe pas de formule fermée à côté qui donnerait la valeur finale plus vite. Deux façons de calculer un capital donneraient deux vérités à tenir d'accord, ce que le §4.6 bis refuse déjà pour la valeur estimée d'un support.
- **Les versements repris sont amortis au mois** (`monthlyEquivalent`, §4.2), comptés **en net** — une reprise récurrente se soustrait d'un versement récurrent — et une règle **arrêtée** ne compte plus. C'est la convention de tout le reste de l'app : deux amortissements différents feraient deux chiffres sous le même mot d'un écran à l'autre.

**L'étage suivant est nommé, pas promis.** Un **plan** — un support cible, un versement prévu, une hypothèse, un horizon — comparerait chaque mois le prévu au confirmé, montrerait l'adhérence, l'écart cumulé, et la projection **recalée depuis le capital réel** plutôt que depuis le plan théorique. C'est ce qui ferait d'une calculatrice une raison d'ouvrir l'app, et c'est un chantier à part (§2) : il ajoute une entité au document, donc des migrations et une lecture de plus sur chaque écran d'épargne. Brancher l'origine sur l'épargne l'a **rapproché sans l'entamer** — le simulateur sait maintenant d'où il part, il ne sait toujours pas suivre. La seule contrainte que le plan pose dès maintenant est qu'un scénario du simulateur doit pouvoir y être promu sans re-saisie, ce que quatre nombres, une nature de taux et une origine permettent par construction. **Aucun champ n'est posé d'avance**, et l'écran ne promet aucune date : il dit seulement qu'il lit l'épargne sans la suivre.

Le mot « plan d'épargne » entre en collision avec le PEL, le PER et le PEE, et le nom du second étage reste donc à trouver. « Projections » nomme le premier sans ambiguïté — c'est ce que l'écran fait, et rien d'autre.
...
- **Migrations** : chaque changement de forme incrémente `schemaVersion` et ajoute une étape déterministe. Un export ancien reste importable. La migration v8 sépare le support de la catégorie : elle crée **un support par paire (catégorie d'épargne, personne) réellement employée** — pas un de plus —, relie chaque `Entry`, `Recurrence` et `Advance` d'épargne au sien, et **n'invente aucun capital** : les supports naissent sans valorisation, parce que rien dans un document v7 ne dit ce que le livret valait, et que zéro serait une information financière réelle. Ce qui n'est à personne reste sans support : un support est toujours à quelqu'un, et lui inventer un porteur attribuerait à quelqu'un une épargne qu'il n'a pas faite. La migration v9 ajoute `SavingSupport.pace` et **n'écrit rien** : c'est la seule du lot dont l'absence n'est pas remplacée par une valeur par défaut à l'import, parce qu'un document d'avant le champ n'a jamais répondu à la question — poser « annuel » sur chaque support ferait passer un silence pour autant de choix, que l'écran de modification affirmerait ensuite venir de quelqu'un. L'app lit « annuel » sans l'écrire (§4.6 bis).
...
- **Jeu d'exemple** : un document complet — trois personnes aux revenus très inégaux, six crédits dont trois soldés, six avances dont quatre entièrement reconstituées, huit supports d'épargne dont un archivé et un sans relevé, **cinq ans d'historique** — chargeable en un clic. Ce qu'il contient n'est pas une collection de lignes vraisemblables mais une **liste d'états**, chacun retenu parce qu'un écran s'efface sans lui. Une app neuve n'a rien à montrer : pas de courbe, pas de répartition, pas de capital restant dû, et tout ce qui fait son intérêt demande des mois de données que personne ne saisit pour décider s'il va s'en servir. **Cinq ans, parce qu'une durée n'est pas une quantité** : c'est ce qu'il faut pour qu'un crédit aille à son terme et qu'un autre le remplace, pour qu'un foyer locataire achète — le loyer cesse, une mensualité, une taxe foncière et une assurance doublée le remplacent —, pour qu'un alternant soit embauché et fasse basculer le prorata des charges communes, pour que la crèche cède la place à la cantine, et pour qu'un prix montre cinq paliers plutôt qu'un changement isolé. Le jeu est **construit à la date du jour**, jamais figé : le mois courant a ses échéances, l'historique remonte derrière, les comparatifs ont leurs cinq années. Il remplace les données comme un import, donc **double** confirmation — sauf au premier lancement, où rien n'a encore été enregistré et où faire confirmer la perte de rien n'apprendrait qu'une chose, que les questions de cette app ne veulent rien dire.
...
- **Sauvegardes locales** — cinq instantanés tournants, un par jour de saisie, chacun portant l'état d'avant les modifications du jour, restaurables depuis « Plus → Sur cet appareil ». Elles ne remplacent pas un export : elles vivent dans le même navigateur.
