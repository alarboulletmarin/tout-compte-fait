...
- Projections d'épargne : la trajectoire de ses propres supports sur plusieurs années — capital estimé, versements des récurrences, une hypothèse de taux par support — et un simulateur à taux constant sur des chiffres libres, jusqu'à trois hypothèses comparées, en euros courants ou constants
...
**Hors périmètre** — suivi de l'écart entre une projection et le réel mois après mois, objectifs d'épargne datés et suivis, allocation de portefeuille, conseil d'investissement, modèle fiscal français (enveloppes, prélèvement forfaitaire, prélèvements sociaux) et presets de frais par support, simulation stochastique (Monte Carlo), données de marché, comptes bancaires multiples, import de relevés bancaires, synchronisation bancaire, budgets par enveloppe, multi-devise, patrimoine hors épargne, solde roulant entre membres (une dette entre personnes qui court de mois en mois jusqu'à ce qu'un geste la solde ; la régularisation, elle, corrige le mois suivant et s'arrête là).

**Les projections sont entrées dans le périmètre, et pas les plans.** La liste disait « plans et projections d'épargne, rendement attendu, intérêts composés, scénarios prudent/central/optimiste », c'est-à-dire cinq façons de nommer un seul chantier — et l'argument qui les tenait dehors était juste pour un seul d'entre eux. Un **plan** lit les données : il compare le prévu au confirmé mois après mois, donc il ajoute une entité au document, des migrations, et une lecture de plus sur chaque écran d'épargne. Une **projection** ne lit rien : elle prend quatre nombres tapés à la main et les capitalise. Ce n'est pas un demi-plan, c'est un autre objet — et le refuser laissait à l'app un trou qu'aucun autre écran ne comble, alors que les outils qui le comblent ailleurs le font mal exprès (§4.6 ter).

**La projection lit désormais le document ; elle continue de ne rien y écrire.** L'argument ci-dessus tenait sur une opposition trop nette : « un plan lit les données, une projection ne lit rien ». Lire n'est pas écrire, et c'est l'écriture qui coûtait — une entité, des migrations, une lecture de plus sur chaque écran d'épargne. Une projection **qui part des vrais comptes** ne demande rien de tout cela : elle appelle les fonctions qui existent déjà — le capital estimé d'un support, l'équivalent mensuel d'une récurrence — et n'ajoute pas un champ au modèle. Elle est aussi la seule chose que cette app puisse faire et qu'aucun simulateur ne fera : une banque connaît le solde d'un livret et ignore ce qu'on y verse, un simulateur connaît le versement et ignore le solde. Ici les deux bouts sont là, donc la question « ça donne quoi dans dix ans » ne demande **aucune saisie** — sauf le taux, que rien ne peut deviner (§4.6 ter).

**Aucun champ n'est posé pour autant.** Le taux, sa nature et le versement qu'on essaie à la place du vrai vivent **hors du document**, en confort local : un `expectedReturn` sur un `SavingSupport` serait une promesse dans le modèle, lue par un assistant et par la documentation comme un réglage qui fait quelque chose, et il voyagerait dans les exports comme un fait alors qu'il n'est qu'une opinion du jour.

Ce qui reste dehors est le **suivi de l'écart** : comparer chaque mois le prévu au confirmé, montrer l'adhérence, l'écart cumulé, et recaler la trajectoire. Celui-là ajoute bien une entité — il faut mémoriser ce qui était prévu pour pouvoir dire qu'on s'en écarte — donc des migrations et une lecture de plus partout. L'écran projette ; il ne mesure pas encore (§4.6 ter).
...
### 4.6 ter Projections — mes supports, et des chiffres libres
...
**Deux lectures, une adresse, et une seule chose qui les sépare : d'où viennent les chiffres.** Une bascule en tête d'écran, comme partout ailleurs dans l'app quand deux lectures du même objet coexistent.

- **Mes supports** part de ce que l'app sait déjà : le capital estimé de chaque support, et les versements que ses récurrences posent. C'est la seule chose que cette app puisse faire et qu'aucun simulateur ne fera — une banque connaît le solde d'un livret et ignore ce qu'on y verse, un simulateur connaît le versement et ignore le solde. Ici les deux bouts sont là, donc la question « ça donne quoi dans dix ans » ne demande **aucune saisie**, sauf le taux.
- **Chiffres libres** ne lit rien du tout : quatre nombres tapés à la main. Elle garde sa raison d'être — on veut pouvoir poser une question sur un capital qu'on n'a pas encore et un versement qu'on n'a pas programmé.

**Celle qui a quelque chose à montrer s'ouvre la première.** Tant que personne n'a choisi, l'écran ouvre « Mes supports » dès qu'un support du foyer a un relevé, et « Chiffres libres » sinon — un écran qui accueillerait un nouveau venu par « tu n'as rien à projeter » serait un écran qui refuse de servir. Un choix explicite est gardé et l'emporte ensuite : sans cette distinction, qui préfère le simulateur se ferait renvoyer sur ses comptes au premier relevé posé.

**Aucune des deux n'écrit dans le document.** Rien de ce qu'on tape ne ressort dans un export ni dans le schéma qu'on donne à un assistant. Un taux est une **question qu'on pose**, pas un fait du foyer, et la réponse change dès qu'on change d'avis. Les réglages — l'horizon, l'inflation, et le taux de chaque support avec le versement qu'on lui essaie — sont gardés en confort local, hors du document, comme le thème : ce qui décrit cet appareil-ci ne voyage pas dans un fichier exporté. Le pire qu'on risque à les perdre est de retaper un taux. C'est aussi ce qui évite un `expectedReturn` sur un `SavingSupport`, que le §2 refuse.

**Ce que « Mes supports » lit, et d'où.**

- **Le point de départ est la valeur estimée** d'un support — dernier relevé plus les mouvements confirmés depuis —, par la fonction qui rend déjà le chiffre de la tuile Capital et de la fiche du support (§4.6 bis). Trois écrans, un calcul : ils ne peuvent pas diverger d'un centime. Ce chiffre **ne s'édite pas ici** : c'est le seul de la ligne qui soit un fait, et le corriger se fait par un relevé, là où les relevés se posent.
- **Le versement est celui des récurrences**, à l'équivalent mensuel de la fiche d'une récurrence, compté en **net** — une règle de reprise retranche. Les règles arrêtées sortent ; un montant variable n'est **pas deviné** : il est compté à part et l'écran dit ce qu'il n'a pas mis dans la courbe.
- **Le versement, lui, s'essaie.** Il est prérempli et modifiable — « et si je mettais 300 au lieu de 200 » —, et une ligne dont le versement est simulé le **dit**, rappelle le vrai chiffre et propose de le rétablir : une simulation qui ne se distingue pas d'un fait est une simulation qui ment. Rien n'est écrit dans la récurrence.
- **Un support sans relevé n'est pas projeté**, et surtout pas depuis zéro : il est nommé, compté à part, et l'écran mène au relevé groupé. Zéro est une information financière réelle — un livret vidé — quand l'absence de relevé ne dit rien du tout, et un patrimoine faux présenté comme exact est pire que pas de chiffre (§4.6 bis). C'est aussi la porte du **point de départ** pour qui n'en a pas encore posé.
- **Un support archivé n'est pas projeté** non plus : un compte clôturé ne sera plus alimenté, et le faire grossir sur dix ans serait une trajectoire qui n'existe pas.
- **La lecture est individuelle**, comme tout ce qui touche à l'épargne : le bandeau ne propose que des personnes, une est toujours active, et le total porte son nom. Deux personnes qui ont 12 000 € et 8 000 € de côté n'ont pas « 20 000 € » (§4.6 bis). Le bandeau n'a pas de mois : une trajectoire sur dix ans ne dépend pas du mois qu'on regarde ailleurs.
- **Ce qui entre dans le modèle s'écrit au centime, ce qui en sort s'arrondit.** Le capital estimé et le versement réel sont des faits, et les arrondir empêcherait de reconnaître ses propres chiffres — un versement de 254,37 € s'afficherait « 250 € ». Les montants d'arrivée, les jalons et le total sortent d'un modèle : ils s'arrondissent et portent leur « ≈ ».

**Les entrées de « Chiffres libres ».**

- **Capital de départ**, zéro par défaut ; **versement mensuel** ; **durée**, avec quatre raccourcis — 5, 10, 15, 20 ans — et un champ libre borné à 50 ans. Les raccourcis règlent le champ, ils ne le remplacent pas : sans lui, un horizon de sept ans serait inatteignable. Cinquante ans est une borne de **sens**, pas de calcul — au-delà, ni le taux, ni l'inflation, ni la personne qui lit ne seront les mêmes.
- **Taux annuel, saisi net de frais et de fiscalité.** L'app ne modélise ni prélèvement forfaitaire, ni prélèvements sociaux, ni frais de gestion : un texte d'aide dit comment estimer son taux net — le rendement espéré, moins les frais annuels, moins la fiscalité applicable aux gains. C'est une simplification **assumée et écrite** : la fiscalité française bouge, et un barème figé dans le code serait faux dans deux ans. Un barème périmé se lit comme un calcul faux.
- **Nature du taux** : **garanti** (livret réglementé, fonds euros) ou **hypothèse** (PEA, compte-titres, unités de compte). Elle ne change aucun calcul — elle change ce que le chiffre **engage** : un taux garanti est une propriété du contrat, une hypothèse n'engage que qui la pose. La distinction passe par le mot et par la forme du trait, **jamais par la couleur seule** (DS §2.3).
...
**L'horizon et les euros constants sont communs aux deux lectures** : ce sont des façons de lire, pas des propriétés de la question posée, et deux contrôles pour la même chose auraient fini par ne plus proposer les mêmes raccourcis.
...
Cette limite ne vaut **que** pour les chiffres libres, et le tableau de jalons de « Mes supports » n'y déroge pas par laxisme : là-bas les colonnes sont des hypothèses **concurrentes**, qu'on compare et que trois traits bornent ; ici ce sont des **comptes**, qui s'additionnent, et le tableau défile déjà dans son cadre. Le graphique, lui, ne trace qu'**une** courbe — le capital total — et laisse le détail au tableau : c'est ce qui évite d'avoir à trouver une septième couleur de trait pour un septième livret.
...
**Les sorties**, les mêmes des deux côtés à une chose près : sur « Mes supports », la courbe unique est le **capital total**, l'aire porte le capital d'aujourd'hui **et** les versements — la ligne de départ n'y est pas zéro, et l'appeler « versements cumulés » ferait passer un capital déjà là pour de l'argent versé demain —, et le tableau de jalons porte une colonne par compte plus le total. Le trait y est plein quand tout ce qu'il additionne est garanti, tireté dès qu'une hypothèse y entre : une somme qui contient un PEA n'a rien de garanti, même si le livret d'à côté l'est.

- Une **courbe par scénario**, et l'aire des **versements cumulés** en dessous. L'aire plutôt qu'un trait : elle découpe le graphique en deux lectures qui n'ont pas besoin d'être expliquées — ce qui vient de la poche, ce qui vient du taux. L'échelle part de **zéro**, contrairement à la courbe d'un support (§4.6 bis), et elle le doit : une aire mesurée depuis une base flottante ne dit rien.
- Un **tableau de jalons** sous le graphe, un rang par quart d'horizon. Il n'est pas un doublon : une courbe ne se lit pas au chiffre près, et pas du tout au doigt — les périodes d'un tracé ne font pas 44px. C'est lui la lecture textuelle que le §5 exige de tout graphique. Des quarts d'horizon plutôt qu'une liste fixe 5/10/15/20 : sur quarante ans une liste fixe laisserait vingt années sans repère, sur trois ans elle ne rendrait rien — et sur vingt ans les quarts **redonnent** 5/10/15/20.
...
Il n'est **pas** un cinquième rang de « Gérer », et c'est la distinction qui fait exister le groupe : « Gérer » range ce qui **décide** de ce que le budget calcule — les récurrences, les supports, le partage, les crédits —, quand une projection ne décide de rien. Qu'elle lise désormais le document n'y change rien : lire n'est pas décider, et rien de ce qu'on pose ici ne modifie un seul chiffre du mois. Le titre est un verbe comme les autres, « Simuler » et non « Calculateurs » : les quatre autres nomment une **intention**, quand « Calculateurs » nommerait une catégorie d'outil, c'est-à-dire ce qu'on a sous la main plutôt que ce qu'on cherche. C'est le reproche exact fait à « Réglages », et le rangement ne le réintroduit pas par la petite porte.
...
L'URL est à la racine et non sous `/epargne/` parce que tout ce qui vit sous ce préfixe est un **objet** de l'épargne — un support, un relevé — et se comporte comme une fiche ; les projections sont une destination pleine, qu'on met en signet. La bascule entre les deux lectures ne voyage pas dans l'URL : ce n'est pas une autre page, c'est la même question sous une autre origine — et le signet doit rendre la lecture qu'on a choisie, que le confort local garde déjà.
...
- **Un seul moteur.** Le tracé, les jalons et le chiffre d'arrivée lisent la **même** série ; il n'existe pas de formule fermée à côté qui donnerait la valeur finale plus vite. Deux façons de calculer un capital donneraient deux vérités à tenir d'accord, ce que le §4.6 bis refuse déjà pour la valeur estimée d'un support. La lecture branchée sur les supports n'y fait pas exception : chaque compte passe par la même fonction, et le total est la **somme rang par rang** de leurs séries.
- **Le total ne se reprojette jamais à un taux moyen.** Un livret à 2 % et un PEA à 7 % ne font pas un patrimoine à 4,5 %, et l'écart grandit avec l'horizon. C'est le corollaire direct de la règle précédente : sommer les trajectoires est le seul calcul qui redonne, au centime, ce que les colonnes du tableau annoncent une à une.
- **Le versement des récurrences est ramené au mois par la fonction de leur fiche**, et compté en net. Deux façons de mensualiser une règle donneraient deux chiffres sous le même mot, à un écran d'écart.

**L'étage suivant est nommé, pas promis.** Ce qui manque n'est plus de partir du réel — l'écran le fait — mais d'en **mesurer l'écart** : comparer chaque mois le prévu au confirmé, montrer l'adhérence et l'écart cumulé, et dire si la trajectoire tient. Celui-là ajoute bien une entité au document — il faut mémoriser ce qui était prévu pour pouvoir dire qu'on s'en écarte —, donc des migrations et une lecture de plus sur chaque écran d'épargne, et c'est un chantier à part (§2). **Aucun champ n'est posé d'avance**, et l'écran ne promet aucune date : il dit seulement qu'il projette et ne mesure pas encore.

Le mot « plan d'épargne » entre en collision avec le PEL, le PER et le PEE, et le nom de cet étage-là reste donc à trouver. « Projections » nomme celui-ci sans ambiguïté — c'est ce que l'écran fait, et rien d'autre.
