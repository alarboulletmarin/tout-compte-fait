...
describe('l’écran range chaque question dans sa zone', () => {
  ...
  /* L'écran ne montre jamais que les comptes de la personne lue : répéter son
     nom sur chaque rangée n'apprendrait rien, et la place sert à ce qu'on ne
     savait pas dire — depuis quand ce chiffre n'a pas bougé. C'est la
     ventilation du mois qui le nomme, elle, parce qu'elle compte des `Entry` et
     peut porter deux comptes homonymes de deux personnes. */
  it('ne répète pas le titulaire sur chaque compte', () => {
    seed()
    open()

    const supports = screen.getByText(t.savings.supports).closest('section')
    expect(supports).not.toBeNull()
    expect(within(supports as HTMLElement).queryByText(/Andrea/)).not.toBeInTheDocument()
