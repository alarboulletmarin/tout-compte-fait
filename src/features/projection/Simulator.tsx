/* ============================================================================
 * Le simulateur — quatre nombres tapés à la main, et rien d'autre.
 *
 * **Il ne lit pas le document, et n'y écrit rien.** C'est une calculatrice qu'on
 * interroge : aucune `Entry`, aucun support, aucun relevé n'entre ici, et rien
 * de ce qu'on y tape ne ressort dans un export. La lecture branchée sur les
 * vrais comptes existe désormais à côté (`SupportsProjection`), sous l'autre
 * position de la bascule — mais celle-ci garde sa raison d'être : on veut
 * pouvoir poser une question qui ne parle d'aucun compte existant, avec un
 * capital qu'on n'a pas encore et un versement qu'on n'a pas programmé.
 *
...
export function Simulator({ draft, patch }: { draft: ProjectionDraft; patch: Patch }) {
  ...
  return (
    <>
      <Tile className="gap-4">
        ...
        <YearsField draft={draft} patch={patch} {...(errors.years === undefined ? {} : { error: errors.years })} />
      </Tile>
      ...
      <ConstantMoney draft={draft} patch={patch} {...(errors.inflation === undefined ? {} : { error: errors.inflation })} />
      ...
      {/* Ce que cette lecture-ci ne fait pas, dit là où on la quitte : elle ne
          touche pas au document, et l'autre position de la bascule est celle
          qui part des vrais comptes. */}
      <p className="t-label">{projection.freeNote}</p>
    </>
  )
}
